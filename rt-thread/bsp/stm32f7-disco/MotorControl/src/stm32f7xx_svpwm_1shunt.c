...
* Return         : None
*******************************************************************************/
void SVPWM_1ShuntInit(void)	   //初始化PWM 及ADC 外设 MCU复位后调用
{ 
...
* Return         : None
*******************************************************************************/
void SVPWM_1ShuntCurrentReadingCalibration(void) //存储零电流时采样电路的ADC值 马达启动命令后调用
{
  static u16 bIndex;
...
* Return         : None
*******************************************************************************/
void SVPWM_1ShuntCalcDutyCycles (Volt_Components Stat_Volt_Input) //计算占空比值，配置下一次电流采样时的ADC及定时器模式	 FOC 子程序之后调用
{
...
* Return         : None
*******************************************************************************/
Curr_Components SVPWM_1ShuntGetPhaseCurrentValues(void)	  //计算相A 及 相B的电流值  在ADC转换完成后及FOC 子程序之前调用
{
...
* Return         : None
*******************************************************************************/
u8 SVPWMEOCEvent(void) //在ADC转换结束事件的ISR 中执行的子程序 在ADC转换结束事件的ISR 中调用
{
	#if 0
  if (bDistEnab == 1)
  {
...
* Return         : None
*******************************************************************************/
void SVPWMUpdateEvent(void)	  //在Update 事件的ISR 中执行的子程序 在Update 事件的ISR 中调用
{
