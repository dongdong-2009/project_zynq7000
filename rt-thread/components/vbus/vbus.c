...
static void _bus_out_entry(void *param)
{
                             ...
                             RT_WAITING_FOREVER) == RT_EOK)
    {
        ...
        if (dpkg.finished)
        {
            _vbus_indicate(RT_VBUS_EVENT_ID_TX, dpkg.id);//·¢ËÍÖÐ¶ÏÍ¨Öª
        }
    }
    RT_ASSERT(0);
}
...
void rt_vbus_close_chn(unsigned char chnr)
{
    ...
    if (err == RT_EOK)
        /* wait for the ack */
        rt_vbus_listen_on(chnr, 10 * RT_TICK_PER_SECOND);   /*µÈ´ý½ÓÊÕÍêÊý¾Ý*/
...
{
    if (RT_VBUS_OUT_RING->blocked)   /*ÕýÔÚÐ´¶ÓÁÐ£¬¾ÍÍË³ö*/
...
/*
VBUS³õÊ¼»¯
@param 1, Êä³ö»·ÐÎ¶ÓÁÐµÄÖ¸Õë
@param 2, ÊäÈë»·ÐÎ¶ÓÁÐµÄÖ¸Õë
*/
int rt_vbus_init(void *outr, void *inr)
{
    ...
    for (i = 0; i < ARRAY_SIZE(_sess); i++)/*sess×öÉ¶ÓÃå**/
    {
        _sess[i].req = RT_NULL;
        _sess[i].st  = SESSIOM_AVAILABLE;
    }
